...
pub fn setup_panel_window(app: &AppHandle<Wry>) {
    ...
    if let Some(win) = app.get_webview_window("copy-panel") {
        ...
        if win.label() == "copy-panel" {
            ...
            println!("Window found but label mismatch: expected 'copy-panel', got '{}'", win.label());
        }
    } else {
    ...
    if let Some(settings_win) = app.get_webview_window("settings") {
        println!("settings window found - label: '{}' - keeping as regular window", settings_win.label());
    ...
    match panel_name.as_str() {
        "copy-panel" => {
            if let Some(win) = app.get_webview_window("copy-panel") {
                ...
                println!("Opening NSPanel at physical position: ({}, {})", physical_x, physical_y);
